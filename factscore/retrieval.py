...
class DocDB(object):
    ...
    def __init__(self, db_path=None, data_path=None, tokenizer_path=None):
        ...
        if len(cursor.fetchall())==0:
            ...
            self.build_db(self.db_path, data_path, tokenizer_path)
    ...
    def build_db(self, db_path, data_path, tokenizer_path):
        from transformers import RobertaTokenizer
        #jh, mobert-large
        self.tokenizer_path = tokenizer_path
        if tokenizer_path is None:
            tokenizer = RobertaTokenizer.from_pretrained("roberta-large") # org
        else:
            tokenizer = RobertaTokenizer.from_pretrained(self.tokenizer_path)
...
class Retrieval(object):
    ...
    def __init__(self, db, cache_path, embed_cache_path,
                 retrieval_type="gtr-t5-large", batch_size=None, ckp_path=None):
        self.db = db
        self.cache_path = cache_path
        self.embed_cache_path = embed_cache_path
        self.retrieval_type = retrieval_type
        self.batch_size = batch_size
        assert retrieval_type=="bm25" or retrieval_type.startswith("gtr-") or retrieval_type.startswith("cross") or retrieval_type.startswith("paraphrase")
    ...
    def load_encoder_reranker(self):
        from reranking_lce_202402.reranking import RerankingForInference
        checkpoint_path= self.ckp_path
        encoder = RerankingForInference.from_pretrained(checkpoint_path)  
        encoder = encoder.cuda()
    ...
    def get_crossencoder_passages(self, topic, retrieval_query, passages, k):
        if self.encoder is None:
            self.load_encoder_reranker()

        passage_list = [psg["title"] + " " + psg["text"].replace("<s>", "").replace("</s>", "") for psg in passages]

        if len(passage_list) > 1:
            question_list = []
            for ii in range(len(passage_list)):
                question_list.append(retrieval_query)
        else: # passage가 하나인 경우가 많음.
            question_list = [retrieval_query]

        assert len(question_list) == len(passage_list)

        inputs = self.encoder.tokenize(question_list, passage_list, return_tensors='pt',padding=True,  truncation=True, max_length=512)
        inputs = {key: value.to(self.device) for key, value in inputs.items()}

        num_samples = len(question_list)
        score_list=[]
        batch_inputs = {key: value[0:num_samples] for key, value in inputs.items()}
        with torch.no_grad():
            score = self.encoder(batch_inputs).logits
            score_list+=score.detach().cpu().numpy().reshape(-1).tolist()
        
        score_list = [-1*round(score, 4) for score in score_list]
        indices = np.argsort(score_list)[:k]
    ...
    def get_passages(self, topic, question, k):
        retrieval_query = topic + " " + question.strip() # original
        ...
        if cache_key not in self.cache:
            ...
            elif self.retrieval_type.startswith("cross"):
                retrieval_query = question.strip() # jh
                cache_key = topic + "#" + retrieval_query # jh
                self.cache[cache_key] = self.get_crossencoder_passages(topic, retrieval_query, passages, k)
            else:
                self.cache[cache_key] = self.get_gtr_passages(topic, retrieval_query, passages, k)
            
            # assert len(self.cache[cache_key]) in [k, len(passages)] #passage 수가 적을 수 있어서 블락
            self.add_n += 1
